...
t
    ...
    in
    if needsQuotes then ''"${lib.escape [ "\\" "\"" ] escapedPercent}"'' else escapedPercent;
  ...
  entryToDesktopFile =
    ...
    let
      categories =
        ...
        then
          "Network;WebBrowser"
        else if lib.hasInfix "ghostty" entry.binary || lib.hasInfix "kitty" entry.binary then
  ...
  desktopFileEntries = lib.mapAttrs' (name: entry: {
